...
const EmailVerification = require('../../models/main/EmailVerification.model.js');
const ForgotPasswordVerification = require('../../models/main/forgotPasswordVerification.model.js');
            ...
            bcrypt.hash(cmsusers.password, salt, (err, hash) => {
                if (err) console.error('There was an error', err);
                    ...
                    cmsusers.save().then(user => {
                        ...
                        //send_mail(email, status , req );
                                

                        var query = { email: email },
                        update = { verification_code: rand },
                        options = { upsert: true, new: true, setDefaultsOnInsert: true };

                        // Find the document
                        EmailVerification.findOneAndUpdate(query, update, options, function(error, result) {
                            if (error){
                                res.status(500).send({
                                    message: "Somethin went wrong !!!"
                                });
                            }else{
                                send_mail(email,rand,req); 
                                res.status(200).json({
                                    status: true,
                                    status_code: 200,
                                    message: 'OK',
...
exports.forgotPassword = (req, res) => {
    ...
    // Find the document
    ForgotPasswordVerification.findOneAndUpdate(query, update, options, function(error, result) {
        if (error){
            res.status(500).send({
                message: "Somethin went wrong !!!"
            });
            ...
            res.status(200).json({
                status: true,
                status_code: 200,
                message: 'forgot password verification mail Sent',
                data: {
                    "email": email
                }
            });
...
function send_mail(email, rand, req){
    // documents array
  
        // rand=Math.floor((Math.random() * 100) + 54);
        
        host = req.get('host');
        link = "http://" + req.get('host') + "/api/verify-email/"+ rand;
